...
def build_adapter(kind: str | None = None) -> InferenceAdapter:
    ...
    if kind == "mlx":
        ...
        model = os.environ.get("AXIS_ADAPTER_MLX_MODEL")
        if not model:
            raise RuntimeError(
                "MLX adapter requires AXIS_ADAPTER_MLX_MODEL — "
                "path to a folder containing an mlx-vlm model."
            )
        return MLXAdapter(model_path=model)

    if kind == "transformers":
        from inference.transformers import TransformersAdapter
        model = os.environ.get("AXIS_ADAPTER_TRANSFORMERS_MODEL")
        if not model:
            raise RuntimeError(
                "Transformers adapter requires AXIS_ADAPTER_TRANSFORMERS_MODEL — "
                "either an HF Hub ID or a local checkpoint path."
            )
