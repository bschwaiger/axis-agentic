"""
Coordinator (Anthropic) — Outer loop for the AXIS Agentic pipeline.
...
Uses Anthropic Claude API instead of NVIDIA NIM.

Usage:
    python -m orchestrator.coordinator_anthropic
    python -m orchestrator.coordinator_anthropic --verbose
"""
...
from orchestrator.evaluator_agent_anthropic import run_agent as run_evaluator  # noqa: E402
from orchestrator.analyst_agent_anthropic import run_agent as run_analyst  # noqa: E402
from orchestrator.formatting_anthropic import format_checkpoint  # noqa: E402
...
def run_pipeline(config: dict, verbose: bool = False):
    ...
    print(f"  LLM:      Claude ({os.environ.get('ANTHROPIC_MODEL', 'claude-sonnet-4-6')})")
    print()
...
def _print_results_summary(matrix: dict, out_dir: Path):
    ...
    if lit_path.exists():
        from orchestrator.formatting_anthropic import format_literature_results
...
def main():
    parser = argparse.ArgumentParser(description="AXIS Agentic Coordinator (Anthropic)")
