"""
Analyst Agent (Anthropic) — Agent 2 in the AXIS Agentic pipeline.
...
2. Claude proposes PubMed search queries based on comparative findings
...
Usage:
    python -m orchestrator.analyst_agent_anthropic \
...
def load_tools() -> list[dict]:
    tools_path = PROJECT_ROOT / "tools" / "analyst_tools_anthropic.json"
...
def run_agent(matrix_path: str, task: dict, verbose: bool = False) -> dict:
    ...
    for turn in range(1, MAX_TURNS + 1):
        ...
        # Send tool results back if any
        if tool_results:
            messages.append({"role": "user", "content": tool_results})

        # Check for checkpoint: Claude stops with text containing queries
        if stop_reason == "end_turn" and not checkpoint_done:
            ...
            if text_content and _parse_queries(text_content):
                approved_queries = terminal_checkpoint_queries(text_content)
        ...
        if stop_reason == "end_turn" and checkpoint_done:
            for block in response.content:
                if block.type == "text" and block.text:
                    ...
                    events.emit("claude_text", text=block.text, agent="analyst")
...
def main():
    parser = argparse.ArgumentParser(description="AXIS Analyst Agent (Anthropic)")
