"""
Cockpit (Demo Mode, Anthropic) — Web UI with demo banner for synthetic inference.
...
Claude orchestrates all pipeline steps.

Usage:
    python -m cockpit.app_demo_anthropic
"""
...
from cockpit.app_anthropic import app, _kill_port  # noqa: E402
...
">
  DEMO MODE &mdash; Local Inference Simulated (Anthropic)
  <span style="font-weight:400; margin-left: 12px; opacity: 0.8;">
    Predictions are synthetic. Claude orchestrates all other steps live.
  </span>
</div>
<style>
...
def main():
    parser = argparse.ArgumentParser(description="AXIS Agentic Cockpit (Demo Mode, Anthropic)")
    ...
    print()
    print("  \033[43m\033[30m DEMO MODE (Anthropic) \033[0m")
    print(f"  Cockpit: http://{args.host}:{args.port}")
    print(f"  Inference is synthetic — no server needed.")
    print(f"  Claude orchestrates all pipeline steps live.")
