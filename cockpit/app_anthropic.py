"""
Cockpit (Anthropic) — Lightweight web UI for the AXIS Agentic pipeline.

Identical to app.py but imports from coordinator_anthropic instead of coordinator.

Usage:
    python -m cockpit.app_anthropic                  # start cockpit server only
    python -m cockpit.app_anthropic --run             # start cockpit + launch pipeline
    python -m cockpit.app_anthropic --run --verbose   # with verbose agent output
"""
...
app = FastAPI(title="AXIS Agentic Cockpit (Anthropic)")
...
async def start_pipeline(request: Request):
    ...
    from orchestrator.coordinator_anthropic import AVAILABLE_MODELS, AVAILABLE_DATASETS, run_pipeline
...
async def get_config():
    from orchestrator.coordinator_anthropic import AVAILABLE_MODELS, AVAILABLE_DATASETS
    return JSONResponse({
...
def main():
    parser = argparse.ArgumentParser(description="AXIS Agentic Cockpit (Anthropic)")
    ...
    if not os.environ.get("ANTHROPIC_API_KEY"):
        print("[!] ANTHROPIC_API_KEY not set in .env — the pipeline will fail.")
        print("    Add it to .env: ANTHROPIC_API_KEY=sk-ant-...")

    print(f"  Cockpit: http://{args.host}:{args.port}")
    print(f"  Powered by Claude (Anthropic)")
