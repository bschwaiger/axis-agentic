...
SCHEMA_DOC = """\
Propose a JSON schema describing how to extract (image_path, label) pairs.
...
When source = "manifest_csv":
  ...
  path_column:        column name holding the image_path
  label_column:       column name holding the label
  label_positive:     value of label_column that means positive (label=1)
  label_negative:     (optional) value that means negative (label=0). If
                      omitted, anything != label_positive is 0.

When source = "folder_name_pattern":
  folder_glob:        glob (relative to dataset_path) matching folders whose
                      *name* indicates the label (e.g. "**/*_positive",
                      "class_*", "label=1"). Each image inside a matched
                      folder becomes one manifest row with that folder's
                      label.
  positive_pattern:   regex on folder name; matches => label=1
  negative_pattern:   (optional) regex on folder name; matches => label=0
...
def walk_with_schema(dataset_path: str, schema: dict) -> list[dict]:
    """Apply the schema to the dataset and return image-level manifest rows."""
    root = Path(dataset_path)
    if not root.is_absolute():
        root = PROJECT_ROOT / root
    ...
    if source == "manifest_csv":
        ...
        with open(csv_path) as f:
            for r in csv.DictReader(f):
                image_path = r.get(path_col, "").strip()
    ...
    if source == "folder_name_pattern":
        folder_glob = schema.get("folder_glob")
        if not folder_glob:
            raise ValueError("folder_name_pattern schema requires 'folder_glob'")
        ...
        for current_root, _dirs, files in os.walk(root):
            ...
            if not rel_str or not fnmatch.fnmatch(rel_str, folder_glob):
                continue
            ...
            if pos.search(folder_name):
                ...
                label = 0
            # Enumerate images inside the matched folder, one row per file
            for fname in sorted(files):
                if Path(fname).suffix.lower() in IMAGE_EXTS:
                    image_rel = str(rel / fname)
                    rows.append({"image_path": image_rel, "label": label})
