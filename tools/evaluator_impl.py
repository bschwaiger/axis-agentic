...
def run_inference(dataset_path: str, manifest_path: str, output_path: str) -> dict:
    """Iterate the manifest and call the active inference adapter per image.

    Manifest schema: one row per image, columns image_path + label.
    image_path is interpreted relative to dataset_path.
    """
    ...
    for i, row in enumerate(tqdm(manifest, desc="    Inference", unit="img",
        ...
        if not full_image_path.is_file():
            errors.append(f"Not a file: {full_image_path}")
            continue

        try:
            rel_path = str(full_image_path.relative_to(PROJECT_ROOT))
        except ValueError:
            rel_path = str(full_image_path)
        ...
        try:
            ...
            errors.append(f"Inference failed for {image_path}: {e}")
            results.append({
                "image_path": image_path,
                "ground_truth": label,
                "prediction": None,
